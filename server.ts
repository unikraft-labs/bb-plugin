// bb-plugin-unikraft-cloud — a BB plugin backend entry.
//
...
// surfaces: the Example todos page (app.tsx, over RPC), the `bb unikraft-cloud` CLI
...
export default async function plugin(bb: BbPluginApi) {
  ...
  // `bb plugin config unikraft-cloud`. Add `secret: true` for values like API keys.
  ...
  // The `bb unikraft-cloud` command: what agents (and you) use from a shell. Parsing
  ...
  const usage = [
    "Usage:",
    "  bb unikraft-cloud list [--json]",
    "  bb unikraft-cloud add <title> [--json]",
    "  bb unikraft-cloud done <todo-id> [--json]",
    "  bb unikraft-cloud undo <todo-id> [--json]",
    "  bb unikraft-cloud remove <todo-id> [--json]",
  ].join("\n");
  ...
  bb.cli.register({
    name: "unikraft-cloud",
    summary: "Manage the Unikraft Cloud plugin's example todo list",
    commands: [
      { name: "list", summary: "List todos", usage: "bb unikraft-cloud list [--json]" },
      {
        name: "add",
        summary: "Add a todo",
        usage: "bb unikraft-cloud add <title> [--json]",
      },
      {
        name: "done",
        summary: "Mark a todo done",
        usage: "bb unikraft-cloud done <todo-id> [--json]",
      },
      {
        name: "undo",
        summary: "Mark a todo not done",
        usage: "bb unikraft-cloud undo <todo-id> [--json]",
      },
      {
        name: "remove",
        summary: "Remove a todo",
        usage: "bb unikraft-cloud remove <todo-id> [--json]",
      },
    ],
    async run(argv) {
      ...
      const notFound = (missingId: string) => ({
        exitCode: 1,
        stderr: `No todo with id ${missingId}. Run "bb unikraft-cloud list" to see ids.`,
      });
      const todoId = args[0];
      switch (command) {
        case undefined:
